...
@section('content')
        ...
        <section class="py-5">
            <div class="container px-4 px-lg-5 my-5">
                ...
                <div class="col-md-6"><img class="card-img-top mb-5 mb-md-0" src="{{$items->image}}" alt="..." /></div>
                    <div class="col-md-6">
                        <!-- <div class="small mb-1">SKU: BST-498</div> -->
                        <h1 class="display-5 fw-bolder mb-3">{{$items->name}}</h1>
                        <div class="fs-5 mb-3">
                            @if($items->discount>0)
                                    <span class="text-muted text-decoration-line-through">{{$items->price}}</span>
                                    {{$items->price-((($items->discount)/100)*$items->price)}} MMK
                            @else
                                    {{$items->price}} MMK
                            @endif
                        ...
                        <div class="d-flex product_actions">
                            ...
                            <button class="btn btn-outline-dark flex-shrink-0 addToCart" type="button" data-id="{{$items->id}}" data-name="{{$items->name}}" data-image="{{$items->image}}" data-price="{{$items->price}}" data-discount="{{$items->discount}}">
                        ...
                        <div class="card h-100">
                            ...
                            <img class="card-img-top" src="{{$item->image}}" alt="..." />
                                ...
                                <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                                    <div class="text-center"><a class="btn btn-outline-dark mt-auto" href="{{route('items.show',$item->id)}}">View Option</a>
                                </div>
                            </div>
                        </div>
                    </div>
